    ...
    async def _delete_async():
        ...
        try:
            ...
            if rw_user and rw_user.get("uuid"):
                ...
                logger.info(f"Successfully deleted Remnawave user for telegram_id {telegram_id} (UUID: {user_uuid})")
            else:
                logger.warning(f"No Remnawave user found for telegram_id {telegram_id}, skipping deletion.")
        except Exception as e:
            logger.error(f"Error while deleting Remnawave user for telegram_id {telegram_id}: {e}")
        finally:
            await client.close()
